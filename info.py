...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002221924680').split()] #Channel id for auto indexing ( make sure bot is admin )
...
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1002018748408') # support group id ( make sure bot is admin )
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002018748408') # request channel id ( make sure bot is admin )
...
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002221924680')) #Log channel id ( make sure bot is admin )
